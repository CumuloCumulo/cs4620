...
export default function ProjectsPage() {
  return <SiteShell><main className="projects-page">
    ...
    <div className="project-list">{projects.map((project) => <article key={project.id}>
      ...
      <div className="project-links"><Link href={`/part/${project.part}/${parts.find((part) => part.id === project.part)?.lessons.at(-1)?.slug}#chapter-assignment`}>进入章节作业 →</Link><span>位于 Part {project.part} 末尾</span></div>
    </article>)}</div>
  </main></SiteShell>;
}
